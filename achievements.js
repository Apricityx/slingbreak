(() => {
  ...
  G.achievementCatalog=[
    entry('double','一箭四雕',.25,'同一支箭击碎 4 块砖',s=>s.kills>=4),
    entry('five','势如破竹',.5,'同一支箭击碎 8 块砖',s=>s.kills>=8),
    entry('ten','连破重围',1,'同一支箭击碎 12 块砖',s=>s.kills>=12),
    entry('twenty','一箭封神',1.5,'同一支箭击碎 20 块砖',s=>s.kills>=20),
    entry('bank','借壁杀招',.5,'反弹后击碎一块砖',s=>s.bankKill),
    entry('trick','几何大师',.75,'累计反弹 3 次后击碎砖块',s=>s.trickKill),
    entry('chain','连锁导演',.5,'同一支箭的连锁伤害击碎 3 块砖',s=>s.chain>=3),
    entry('ice','冰点粉碎',.5,'同一支箭击碎 2 块冻结砖',s=>s.frozen>=2),
    entry('mix','元素交响',1,'同一支箭击碎 3 种特殊砖',s=>s.types.size>=3),
    entry('gold','淘金高手',.5,'同一支箭击碎 2 块金矿砖',s=>s.gold>=2),
    entry('air','超长滞空',.5,'飞行 2 秒后仍能击碎砖块',s=>s.airKill),
    entry('core','终结艺术',1,'击碎至少 3 块砖后命中核心',s=>s.core&&s.kills>=3)
  ];
  const history=()=>{
  ...
  G.initAchievementArrow=a=>{
    a.achievement={id:++serial,get kills(){return G.arrowKills(a);},chain:0,frozen:0,gold:0,types:new Set(),unlocked:new Set(),bounces:0,base:0,paid:0,mult:1,baseMult:1,bountyMult:1,cap:G.comboMultiplierCap(),capKills:G.comboCapKills()};
  };
  function evaluate(a){
  ...
  G.awardBrick=(b,depth)=>{
    ...
    const s=a.achievement;s.chain+=depth>0?1:0;s.frozen+=b.frozen?1:0;s.gold+=b.type==='gold'?1:0;
