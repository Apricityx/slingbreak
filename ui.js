(() => {
  ...
  const upgrades={power:{name:'弹弓',icon:'crosshair',desc:()=>`拉力 ${Math.round(G.speed()/24*100)}% · 更远射程`},arrow:{name:'箭矢',icon:'move-up-right',desc:()=>`伤害 ${G.damage().toFixed(2)} · 穿透 ${G.penetration()} 块`},brick:{name:'砖块',icon:'blocks',desc:()=>`价值 +${Math.round((G.valueMultiplier()-1)*100)}% · 特殊率 ${(G.specialRate()*100).toFixed(1)}%`},comboCap:{name:'连击倍率上限',icon:'trending-up',desc:()=>`当前上限 ×${G.comboMultiplierCap()} · 每级 +1`} };
  for(const [key,u] of Object.entries(upgrades)){
     const el=document.createElement('div');el.className='upgrade';el.hidden=key==='comboCap'&&G.state.up.brick<30;el.innerHTML=`<span class="upgrade-icon"><i data-lucide="${u.icon}"></i></span><div><div class="upgrade-title"><b>${u.name}</b><small id="${key}-level"></small></div><p class="upgrade-desc" id="${key}-desc"></p></div><button class="buy-button" id="buy-${key}" aria-label="升级${u.name}"><i data-lucide="plus"></i><span></span></button>`;$('upgrades').append(el);$('buy-'+key).onclick=()=>G.buy(key);
  }
  ...
  G.ui=()=>{
    ...
    if(!G.drag)$('power-readout').querySelector('b').textContent='0%';
     for(const [key,u] of Object.entries(upgrades)){
       const row=$('buy-'+key).closest('.upgrade');row.hidden=key==='comboCap'&&G.state.up.brick<30;
